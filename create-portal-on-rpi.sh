...
# Copy and paste this entire script into your RPi terminal

sudo tee /opt/camera-agent/provisioning_portal.py > /dev/null << 'PYTHON_EOF'
#!/usr/bin/env python3
"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if config:
                    ...
                    subprocess.run(["systemctl", "enable", "camera-agent"], check=False)
                    subprocess.run(["systemctl", "start", "camera-agent"], check=False)
...
def index():
    ...
    return render_template_string(
        PORTAL_HTML, 
        hostname=portal.device_info['hostname'], 
        mac_address=portal.device_info['macAddress'], 
        serial_number=portal.device_info['serialNumber'], 
        ssid=portal.hotspot_ssid,
