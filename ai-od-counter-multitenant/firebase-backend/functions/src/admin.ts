 ...
 */
export const createSuperadminFromGoogle = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (hasSuperadmins) {
      ...
      if (!context.auth.token.role || context.auth.token.role !== 'superadmin') {
        throw new functions.https.HttpsError(
          'permission-denied',
          'Only superadmins can create additional superadmin accounts'
        );
      }
    }

    const userId = context.auth.uid;
    const userEmail = context.auth.token.email as string;
    const userName = context.auth.token.name as string || userEmail?.split('@')[0] || 'User';
    ...
    throw new functions.https.HttpsError(
      'internal',
      error.message || 'Failed to create superadmin'
    );
  }
});
...
export const createSuperadmin = functions.https.onCall(async (data, context) => {
  ...
  if (!email || !password || !name || !companyName) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Missing required fields: email, password, name, companyName'
    );
  }
  ...
  try {
    ...
    try {
      ...
      if (existingSuperadmin.exists) {
        throw new functions.https.HttpsError(
          'already-exists',
          'This email is already registered as a superadmin'
        );
      }
...
export const createSubadmin = functions.https.onCall(async (data, context) => {
  ...
  if (!context.auth || !context.auth.token.role || context.auth.token.role !== 'superadmin') {
    throw new functions.https.HttpsError(
      'permission-denied',
      'Only superadmins can create subadmin accounts'
    );
  }
  ...
  try {
    ...
    throw new functions.https.HttpsError(
      'internal',
      error.message || 'Failed to create subadmin'
    );
  }
});
